...
-- LOGIC:
-- FIND RECEIPTS WITH MANY DETAILS (including * StatementId *) and delete others
    ...
    BEGIN
        ...
        ELSE
            BEGIN
                ...
                ORDER BY Id;
                IF (@id=-1)
                    PRINT ('NOT DUPL WITH StatementId FOR ' + @product_id + ' with ' + LEFT(@checksum, 10) + '...')
                ELSE
                    BEGIN
                        SET @iter_ix += 1; -- count iters when found StatementId
                        -- DETECT DUPLICATES:
