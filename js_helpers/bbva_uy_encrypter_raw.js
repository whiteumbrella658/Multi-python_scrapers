...
var s_account = "bbvauruguaydev";
var s = s_gi(s_account);
s_getLoadTime();
/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
s.debugTracking = true;
s.trackingServer = "bbva.112.2o7.net";
s.visitorNamespace = "bbva";
...
function s_doPlugins(s) {
    s.setupFormAnalysis();
    s.getQueryParam();
    s.eVar21 = s.prop11 = s_getLoadTime();
    var ppvArray = s.getPercentPageViewed(s.pageName);
    s.prop40 = ppvArray[0] //contains the previous page name
    s.prop41 = ppvArray[1] //contains the total percent viewed
    s.prop42 = ppvArray[2] //contains the percent viewed on initial load
    s.prop43 = ppvArray[3] //contains the total number of vertical pixels
...
function s_getLoadTime() {
    if (!window.s_loadT) {
        var b = new Date().getTime(), o = window.performance ? performance.timing : 0,
...
function s_gi(un, pg, ss) {
    var c = "s.version='H.26.1';s.an=s_an;s.logDebug=function(m){var s=this,tcf=new Function('var e;try{console.log(\"'+s.rep(s.rep(s.rep(m,\"\\\\\",\"\\\\"
        ...
        m = u.indexOf('Netscape6/'), a, i, j, x, s;
    if (un) {
        un = un.toLowerCase();
