...
6.22.0
more WRONG_CREDENTAILS_MARKERS
6.21.1
upd log msg
6.21.0
...
class AbancaBeAbancaScraper(BasicScraper):
    ...
    scraper_name = 'AbancaBeAbancaScraper'
    ...
    def login(self) -> Tuple[MySession, Response, bool, bool, str]:
        ...
        resp_init = s.get('https://be.abanca.com', headers=self.req_headers, proxies=self.req_proxies)
        ...
        req_params = OrderedDict([
            ...
            ('loginForm:btnAceptar', 'Acceder'),
            ('loginForm_SUBMIT', '1'),
            ('loginForm:_idcl', ''),
            ('javax.faces.ViewState', view_state_param),
        ])

        req_login_url = 'https://be.abanca.com/BEPRJ001/jsp/SECURE_login_es_step1.faces'
        ...
        if 'su dispositivo Token' in resp_logged_in.text:
            self.logger.error(
                "The fin_ent_access {} requires hardware token to access. "
                "It will be marked as inactive (like credentials error).".format(
                    self.db_financial_entity_access_id
                )
            )

        is_credentials_error = any(m in resp_logged_in.text for m in WRONG_CREDENTAILS_MARKERS)
        reason = ''
        # Revise la información recibida en su móvil.
